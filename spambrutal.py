...
try:
	...
	color_table = random.choice(['#00FF00','#00C8FF','#FFFF00','#FFFFFF','#FF8F00','#AAAAAA'])
...
def bot():
	print()
	coli(f'[+] {B}𝐆𝐔𝐍𝐀𝐊𝐀𝐍 𝐃𝐄𝐍𝐆𝐀𝐍 𝐁𝐈𝐉𝐀𝐊 𝐘𝐀 𝐁𝐎𝐂𝐈𝐋 𝐊𝐎𝐍𝐓𝐎𝐋 𝐃𝐀𝐍 𝐉𝐀𝐍𝐆𝐀𝐍 𝐋𝐔𝐏𝐀 𝐂𝐎𝐋𝐈 {P}')
	os.system('clear')
	cetak(panel(f"{P}[01] Spam SMS [[bold green] ON [bold white]]\n[02] Spam WA [[bold green] ON [bold white]]",width=90,title=f"[bold green]Menu Bot",padding=(0,8),style=f"{color_table}"))
	bray = input(f'[{H}•{P}] Pilih Menu Spam : ')
	if bray in(''):
...
def spam_sms():
	global nomor 
	cetak(panel(f'''   Masukan Nomor Target Yang Ingin Di Spam Contoh : +6281234567xxx''',width=90,padding=(0,8),style=f"{color_table}"))
	nomor = input(f"[{H}•{P}] Input No Hp : +62").replace("+62","")
	if nomor == "":
		pass
	else:
		while True:
...
class sxp_sms:
	...
	def main(self):
		...
		cetak(panel(f"Sukses Spam SMS Ke No : +62{nomor}",width=90,padding=(0,2),style=f"{color_table}"))
...
class sxp_wa:
	...
	def main(self):
		...
		self.wa_otp_5(nomor)
		self.wa_otp_6(nomor)
		cetak(panel(f" Sukses Spam WA Ke No : {K2}+62{nomor}",width=90,padding=(0,2),style=f"{color_table}"))
#-----------------------[ SYSTEM-CONTROL ]--------------------#
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/HASIL-AKUN')
	except:pass
